(() => {
  ...
  function selectionText(source){
    ...
    const lines=['قائمة التحصيل — '+source.name,'التاريخ: '+reportDate(),'عدد المحلات: '+rows.length];if(hasActivity(source)&&filter.type!=='all')lines.push('فلتر النشاط: '+activityLabel(filter));lines.push('');
  ...
  async function shareSelected(){const source=await requireSelected();if(!source)return;const text=selectionText(source);if(!navigator.share){const ok=await writeClipboard(text);window.MyToolBottomNav?.toast(ok?'المشاركة غير متاحة؛ تم نسخ القائمة بدلًا منها':'تعذرت المشاركة والنسخ',!ok);return}try{await navigator.share({title:'قائمة التحصيل — '+source.name,text});window.MyToolBottomNav?.toast('تم فتح المشاركة')}catch(error){if(error?.name==='AbortError')return;const ok=await writeClipboard(text);window.MyToolBottomNav?.toast(ok?'تعذرت المشاركة؛ تم نسخ القائمة بدلًا منها':'تعذرت المشاركة والنسخ',!ok)}}
