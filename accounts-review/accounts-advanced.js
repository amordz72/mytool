(() => {
  ...
  function customerCardHtml(g){const members=g.shared?'<div class="accounts-customer-members">'+g.members.map(customerMemberHtml).join('')+'</div>':'';return '<article class="accounts-customer-card'+(g.shared?' is-shared':'')+'"><header><div><div class="accounts-name-row"><strong>'+escapeHtml(g.name)+'</strong>'+netBadge(g.balance,g.debt)+'</div><small>'+(g.shared?g.members.length+' حسابات مشتركة':'غير مشترك')+'</small></div>'+(g.shared?'<span class="shared-badge">مشترك</span>':'')+'</header>'+members+'<footer><span class="metric debt">الدين <b>'+displayNumber(g.debt)+'</b></span><span class="metric balance">الرصيد <b>'+displayNumber(g.balance)+'</b></span><span class="metric profit">الأرباح <b>'+displayNumber(g.profit)+'</b></span></footer></article>'}
