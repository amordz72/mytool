(() => {
  ...
  async function renderTotal(){
    const data=await totalsData(),tabs=document.querySelector('.tabs');if(!tabs)return;
    ...
    const tabHtml='المجموع الكلي <small>('+data.count+')</small>';if(tab.innerHTML!==tabHtml)tab.innerHTML=tabHtml;
    ...
    const fingerprint=JSON.stringify(data.sources.map(s=>[s.id,s.count,s.balance,s.debt,s.profit]));
    if(panel.dataset.totalFingerprint!==fingerprint){panel.dataset.totalFingerprint=fingerprint;panel.innerHTML='<div class="head-row"><div><h2>المجموع الكلي</h2><div class="meta">جمع مباشر لكل مصدر مستقل، بدون دمج الحسابات المتشابهة بين المنصات.</div></div></div><div class="summary accounts-total-summary"><div class="stat debt"><span>إجمالي الديون</span><strong>'+displayNumber(data.debt)+'</strong></div><div class="stat balance"><span>إجمالي الأرصدة</span><strong>'+displayNumber(data.balance)+'</strong></div><div class="stat profit"><span>إجمالي الأرباح</span><strong>'+displayNumber(data.profit)+'</strong></div></div><div class="accounts-total-list">'+data.sources.map(s=>'<article><div><strong>'+escapeHtml(s.name)+'</strong><small>'+s.count+' حساب</small></div><div><span>رصيد</span><b>'+displayNumber(s.balance)+'</b></div><div><span>دين</span><b>'+displayNumber(s.debt)+'</b></div><div><span>أرباح</span><b>'+displayNumber(s.profit)+'</b></div></article>').join('')+'</div>';}
