(() => {
  ...
  async function renderTotal(){
    ...
    if(panel.dataset.totalFingerprint!==fingerprint){panel.dataset.totalFingerprint=fingerprint;panel.innerHTML='<div class="accounts-all-head"><div><h2>كل العملاء</h2><div class="meta">الحسابات المشتركة تظهر كعميل واحد، وباقي العملاء يظهرون أيضًا.</div></div><button type="button" class="accounts-all-sort sort-'+totalSortMode+'" data-total-sort>ترتيب: '+sortLabel()+' ↓</button></div><div class="summary accounts-total-summary"><div class="stat debt"><span>إجمالي الديون</span><strong>'+displayNumber(data.debt)+'</strong></div><div class="stat balance"><span>إجمالي الأرصدة</span><strong>'+displayNumber(data.balance)+'</strong></div><div class="stat profit"><span>إجمالي الأرباح</span><strong>'+displayNumber(data.profit)+'</strong></div></div><div class="accounts-customers-list">'+(groups.length?groups.map(customerCardHtml).join(''):'<div class="empty">لا توجد حسابات.</div>')+'</div>';panel.querySelector('[data-total-sort]')?.addEventListener('click',()=>{totalSortMode=totalSortMode==='debt'?'balance':totalSortMode==='balance'?'profit':'debt';renderTotal();});}
