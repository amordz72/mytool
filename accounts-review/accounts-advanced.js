(() => {
  ...
  async function customerGroups(data){
    ...
    if(core){try{const st=await core.state();for(const l of st.ls){const members=(l.members||[]).map(m=>core.resolve(m,st.ss)).filter(x=>x?.s&&x?.r).map(x=>({source:x.s,row:x.r}));if(!members.length)continue;members.forEach(x=>used.add(core.rid(x.source.id,x.row)));groups.push({id:l.id,name:l.displayName||customerName(members[0].row),shared:true,members,...customerTotals(members)})}}catch(_e){}}
  ...
  function customerMemberHtml(x){return '<div class="accounts-customer-member"><div><strong>'+escapeHtml(customerName(x.row))+'</strong><small>'+escapeHtml(x.row.username||'—')+' · '+escapeHtml(x.source.name||'—')+'</small></div><div class="accounts-member-money"><span>رصيد <b>'+displayNumber(x.row.balance)+'</b></span><span>دين <b>'+displayNumber(x.row.debt)+'</b></span><span>ربح <b>'+displayNumber(x.row.profit)+'</b></span></div></div>'}
  function customerCardHtml(g){return '<article class="accounts-customer-card'+(g.shared?' is-shared':'')+'"><header><div><strong>'+escapeHtml(g.name)+'</strong><small>'+(g.shared?g.members.length+' حسابات مشتركة':'حساب واحد')+'</small></div>'+(g.shared?'<span class="shared-badge">مشترك</span>':'')+'</header><div class="accounts-customer-members">'+g.members.map(customerMemberHtml).join('')+'</div><footer><span>الرصيد <b>'+displayNumber(g.balance)+'</b></span><span>الدين <b>'+displayNumber(g.debt)+'</b></span><span>الأرباح <b>'+displayNumber(g.profit)+'</b></span></footer></article>'}
  ...
  async function renderTotal(){
    const data=await totalsData(),groups=await customerGroups(data),tabs=document.querySelector('.tabs');if(!tabs)return;
    ...
    const fingerprint=JSON.stringify([totalSortMode,data.sources.map(s=>[s.id,s.count,s.balance,s.debt,s.profit]),groups.map(g=>[g.id,g.balance,g.debt,g.profit,g.members.length])]);
    if(panel.dataset.totalFingerprint!==fingerprint){panel.dataset.totalFingerprint=fingerprint;panel.innerHTML='<div class="accounts-all-head"><div><h2>المجموع الكلي</h2><div class="meta">كل العملاء؛ الحسابات المشتركة تظهر كعميل واحد وباقي العملاء يظهرون أيضًا.</div></div><button type="button" class="accounts-all-sort sort-'+totalSortMode+'" data-total-sort>ترتيب: '+sortLabel()+' ↓</button></div><div class="summary accounts-total-summary"><div class="stat debt"><span>إجمالي الديون</span><strong>'+displayNumber(data.debt)+'</strong></div><div class="stat balance"><span>إجمالي الأرصدة</span><strong>'+displayNumber(data.balance)+'</strong></div><div class="stat profit"><span>إجمالي الأرباح</span><strong>'+displayNumber(data.profit)+'</strong></div></div><div class="accounts-customers-list">'+(groups.length?groups.map(customerCardHtml).join(''):'<div class="empty">لا توجد حسابات.</div>')+'</div>';panel.querySelector('[data-total-sort]')?.addEventListener('click',()=>{totalSortMode=totalSortMode==='debt'?'balance':totalSortMode==='balance'?'profit':'debt';renderTotal();});}
    panel.hidden=!totalMode;
  }
  ...
  async function enterTotal(){totalMode=true;document.querySelectorAll('.tabs .tab').forEach(x=>x.classList.remove('active'));document.getElementById('accountsTotalTab')?.classList.add('active');await renderTotal();hideForTotal();const data=await totalsData();window.MyToolBottomNav?.setActions([{slot:2,icon:'📂',label:'ملف',title:'إضافة أو تحديث ملف',onClick:()=>document.querySelector('input[type="file"][multiple]')?.click()},{slot:4,icon:'📋',label:'نسخ الكلي',title:'نسخ المجموع الكلي',onClick:async()=>{const ok=await copyText(totalText(data));window.MyToolBottomNav?.toast(ok?'تم نسخ المجموع الكلي':'تعذر النسخ',!ok)}}])}
  function exitTotal(){if(!totalMode)return;totalMode=false;document.getElementById('accountsTotalPanel')?.setAttribute('hidden','');document.getElementById('accountsTotalTab')?.classList.remove('active');restoreAfterTotal();setTimeout(()=>window.dispatchEvent(new Event('accounts-total-exit')),60)}
  ...
  function bind(){
    ...
    setTimeout(cleanupFalseUnknowns,800);setTimeout(async()=>{await ensureTotal();if(!totalAutoEntered&&document.getElementById('accountsTotalTab')){totalAutoEntered=true;enterTotal()}},500);
  }
