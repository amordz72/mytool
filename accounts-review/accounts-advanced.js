(() => {
  ...
  function netBadge(balance,debt){const n=(Number(debt)||0)-(Number(balance)||0);if(n>0)return '<span class="customer-net negative">صافي مستحق '+displayNumber(n)+'</span>';if(n<0)return '<span class="customer-net positive">صافي رصيد '+displayNumber(Math.abs(n))+'</span>';return '<span class="customer-net zero">صافي 0</span>'}
  ...
  function totalText(data,groups){
    const major='━━━━━━━━━━━━━━━━━━━━',minor='────────────────────',allNet=(Number(data.debt)||0)-(Number(data.balance)||0);
    const lines=['📊 المجموع الكلي — مراجعة الحسابات','التاريخ: '+reportDate(),'','عدد المصادر: '+data.sources.length,'عدد السجلات الأصلية: '+data.count,'عدد العملاء بعد التجميع: '+(groups||[]).length,'','إجمالي الديون: '+displayNumber(data.debt)+' دج','إجمالي الأرصدة: '+displayNumber(data.balance)+' دج','إجمالي الأرباح: '+displayNumber(data.profit)+' دج',allNet>=0?'الصافي المستحق الكلي: '+displayNumber(allNet)+' دج':'صافي رصيد كلي: '+displayNumber(Math.abs(allNet))+' دج','',major,'📂 ملخص المصادر'];
    (data.sources||[]).forEach((source,index)=>{const sourceNet=(Number(source.debt)||0)-(Number(source.balance)||0);lines.push('',source.name,source.count+' حساب','الدين: '+displayNumber(source.debt)+' دج','الرصيد: '+displayNumber(source.balance)+' دج','الأرباح: '+displayNumber(source.profit)+' دج',sourceNet>=0?'الصافي المستحق: '+displayNumber(sourceNet)+' دج':'صافي رصيد: '+displayNumber(Math.abs(sourceNet))+' دج');if(index<data.sources.length-1)lines.push('',minor)});
    lines.push('',major,'👤 تفاصيل الحسابات');
    (groups||[]).forEach((g,index)=>{const members=g.members||[],net=(Number(g.debt)||0)-(Number(g.balance)||0),sourceNames=[...new Set(members.map(x=>x.source?.name).filter(Boolean))];lines.push('',g.name);if(g.shared)lines.push('مشترك في: '+sourceNames.join('، '));if(g.shared){members.forEach((x,memberIndex)=>{lines.push('',x.source?.name||'مصدر');if(x.row?.username)lines.push('الحساب: '+x.row.username);lines.push('الدين: '+displayNumber(x.row?.debt)+' دج','الرصيد: '+displayNumber(x.row?.balance)+' دج','الأرباح: '+displayNumber(x.row?.profit)+' دج');if(memberIndex<members.length-1)lines.push('····················')});lines.push('','إجمالي الدين: '+displayNumber(g.debt)+' دج','الرصيد المسجل: '+displayNumber(g.balance)+' دج','إجمالي الأرباح: '+displayNumber(g.profit)+' دج',net>=0?'الصافي المستحق: '+displayNumber(net)+' دج':'صافي رصيد: '+displayNumber(Math.abs(net))+' دج')}else{const x=members[0];lines.push('المصدر: '+(x?.source?.name||'—'));if(x?.row?.username)lines.push('الحساب: '+x.row.username);lines.push('الدين: '+displayNumber(g.debt)+' دج','الرصيد: '+displayNumber(g.balance)+' دج','الأرباح: '+displayNumber(g.profit)+' دج',net>=0?'الصافي المستحق: '+displayNumber(net)+' دج':'صافي رصيد: '+displayNumber(Math.abs(net))+' دج')}if(index<(groups||[]).length-1)lines.push('',major)});
    lines.push('',major,'نهاية التقرير');return lines.join('\n')
  }
