(() => {
  ...
  function totalText(data){const lines=['المجموع الكلي — مراجعة الحسابات','التاريخ: '+reportDate(),'عدد المصادر: '+data.sources.length,'عدد السجلات: '+data.count,'إجمالي الديون: '+displayNumber(data.debt),'إجمالي الأرصدة: '+displayNumber(data.balance),'إجمالي الأرباح: '+displayNumber(data.profit),''];for(const s of data.sources)lines.push(s.name+' | '+s.count+' حساب | الرصيد '+displayNumber(s.balance)+' | الدين '+displayNumber(s.debt)+' | الأرباح '+displayNumber(s.profit));return lines.join('\n')}
  ...
  async function enterTotal(){totalAutoEntered=true;totalMode=true;window.dispatchEvent(new Event('accounts-total-selected'));const shared=document.getElementById('accountsSharedPanel');if(shared){shared.hidden=true;shared.style.display='none'}document.querySelectorAll('.tabs .tab').forEach(x=>x.classList.remove('active'));document.getElementById('accountsTotalTab')?.classList.add('active');const total=document.getElementById('accountsTotalPanel');if(total){total.hidden=false;total.style.removeProperty('display')}await renderTotal();hideForTotal();const data=await totalsData();window.MyToolBottomNav?.setActions([{slot:2,icon:'📂',label:'ملفات',title:'اختيار عدة ملفات وتحديث المصادر تلقائيًا',onClick:()=>document.querySelector('input[type="file"][multiple]')?.click()},{slot:4,icon:'📋',label:'نسخ الكلي',title:'نسخ المجموع الكلي',onClick:async()=>{const ok=await copyText(totalText(data));window.MyToolBottomNav?.toast(ok?'تم نسخ المجموع الكلي':'تعذر النسخ',!ok)}},{icon:'📤',label:'مشاركة الكل',title:'مشاركة المجموع الكلي',overflow:true,onClick:()=>shareText('المجموع الكلي — مراجعة الحسابات',totalText(data))}])}
