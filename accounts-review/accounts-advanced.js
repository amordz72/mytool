(() => {
  ...
  function exitTotal(){const wasActive=totalMode;totalMode=false;document.getElementById('accountsTotalPanel')?.setAttribute('hidden','');document.getElementById('accountsTotalTab')?.classList.remove('active');restoreAfterTotal();if(wasActive)setTimeout(()=>window.dispatchEvent(new Event('accounts-total-exit')),60)}
