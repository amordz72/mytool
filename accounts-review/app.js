...
createApp({
  setup(){
    ...
    async function readFiles(event){const files=[...(event.target.files||[])];if(!files.length)return;let ok=0;for(const file of files){try{const analysis=await analyzeFile(file);await processAnalysis(analysis);ok++}catch(error){setMessage(file.name+': '+(error?.message||'تعذر قراءة الملف.'),'err')}}event.target.value='';if(files.length>1&&ok===files.length)setMessage('تمت معالجة '+ok+' ملفات بنجاح.');await nextTick();configureNav()}
    ...
    function configureNav(){
      const actions=[{slot:2,icon:'📂',label:'ملف',title:'إضافة أو تحديث ملف',onClick:pickFile}];
