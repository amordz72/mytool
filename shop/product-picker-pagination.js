...
(function(){
  ...
  function start(){
    ...
    function render(){
      const all=filtered(),pages=Math.max(1,Math.ceil(all.length/pageSize));
    ...
    results.addEventListener('click',e=>{const b=e.target.closest('[data-pick-page]');if(!b)return;e.preventDefault();e.stopPropagation();page+=b.dataset.pickPage==='next'?1:-1;render();},true);
  }
  function boot(){let tries=0;const timer=setInterval(()=>{tries+=1;const select=document.getElementById('purchaseProduct');if(select?.dataset?.enhanced==='1'||tries>50){clearInterval(timer);start();}},120);}
