/* MyTool notes Tasks #82/#91/#92/#95/#102/#104 — pagination, persistent page size, bulk actions, export filenames, image downloads, fixed nav and tabs. */
(function(){
  'use strict';

  const notesToolsScript=document.currentScript;
  const myToolRoot=new URL('../',notesToolsScript?.src||location.href);
  const NAV_VERSION='20260915-2310';
  ...
  function selectedIds(){
    return selectedPicks().map(x=>Number(x.value)).filter(Number.isFinite).sort((a,b)=>a-b);
  }

  function filenameWords(){
    const first=selectedPicks()[0]?.closest('.note')?.querySelector('.note-body')?.textContent||'';
    return first.trim().split(/\s+/).filter(Boolean).slice(0,4).join('-')
      .replace(/[\\/:*?"<>|#%&{}$!'@+=`~^;,]+/g,'')
  ...
  function exportFilename(ext){
    ...
    const idPart=ids.length===1?'note-'+ids[0]:(ids.length?'notes-'+ids.join('-'):'notes');
    const words=filenameWords();
    return 'mytool-'+idPart+(words?'-'+words:'')+'-'+stamp()+'.'+ext;
  }

  function patchLegacyDownloadNames(){
    if(HTMLAnchorElement.prototype.__mytoolNotesPatched)return;
    const nativeClick=HTMLAnchorElement.prototype.click;
    Object.defineProperty(HTMLAnchorElement.prototype,'__mytoolNotesPatched',{value:true});
    HTMLAnchorElement.prototype.click=function(){
      if(this.download==='mytool-notes.txt')this.download=exportFilename('txt');
      else if(this.download==='mytool-notes.json')this.download=exportFilename('json');
  ...
  function runLocalWithoutJump(action){
    ...
    if(message){
      ...
      try{
        ...
        restoreMessage=()=>{
          try{if(hadOwn)message.scrollIntoView=previous;else delete message.scrollIntoView}catch{}
        };
      }catch{}
    }
    action();
  ...
  function ensureStyles(){
    ...
    style.id='notes-list-tools-style';
    style.textContent=`
...
`;
  ...
  function ensurePager(){
    let pager=document.getElementById('notesPager');
  ...
  function renderPager(total){
    ...
    pager.innerHTML='<button type="button" class="outline notes-prev" '+(page<=1?'disabled':'')+'>السابق</button>'+
      '<span class="notes-page-info">'+from+'–'+to+' من '+total+' • صفحة '+page+'/'+pages+'</span>'+
      '<button type="button" class="outline notes-next" '+(page>=pages?'disabled':'')+'>التالي</button>'+
      '<label class="notes-page-size">عدد الملاحظات <select data-visual-choice="off">'+PAGE_SIZES.map(n=>'<option value="'+n+'" '+(n===pageSize?'selected':'')+'>'+n+'</option>').join('')+'</select></label>';
    pager.querySelector('.notes-prev').onclick=()=>{if(page>1){page--;refresh()}};
    pager.querySelector('.notes-next').onclick=()=>{if(page<pages){page++;refresh()}};
    pager.querySelector('select').onchange=e=>{
      const value=Number(e.target.value);
      if(!PAGE_SIZES.includes(value))return;
      pageSize=value;page=1;savePageSize(value);refresh();
    };
  }
  ...
  function ensureSelectionBar(){
    ...
    bar.id='notesSelectionBar';
    bar.className='notes-selection-bar';
    bar.setAttribute('role','region');
    bar.setAttribute('aria-label','إجراءات الملاحظات المحددة');
    ...
    bar.querySelectorAll('[data-bulk]').forEach(button=>button.onclick=()=>runLocalWithoutJump(()=>document.getElementById(button.dataset.bulk)?.click()));
    bar.querySelector('.clear-selection').onclick=()=>{document.querySelectorAll('.pick:checked').forEach(x=>x.checked=false);updateSelectionBar()};
  ...
  function updateSelectionBar(){
    ...
    bar.querySelector('.notes-selection-count').textContent=count+' محدد';
  ...
  async function downloadImage(img,button,index){
    const article=img.closest('.note');
    const noteId=article?.querySelector('.pick')?.value||'note';
    const url=img.dataset.url||img.src;
    const original=(img.alt||'').trim();
    const originalExt=(original.match(/\.([a-zA-Z0-9]{2,5})$/)||[])[1]?.toLowerCase()||'jpg';
    ...
    try{
      ...
      a.href=objectUrl;
      a.download='mytool-note-'+noteId+'-image-'+index+'-'+stamp()+'.'+extensionFromBlob(blob,originalExt);
      a.click();
      ...
      a.target='_blank';
      a.rel='noopener';
      a.download='mytool-note-'+noteId+'-image-'+index+'-'+stamp()+'.'+originalExt;
      a.click();
      console.warn('MyTool note image fallback download',error);
    }finally{
  ...
  function enhanceImages(article){
    const images=[...article.querySelectorAll('.images img')];
    images.forEach((img,index)=>{
      if(img.dataset.downloadReady)return;
      img.dataset.downloadReady='1';
      const wrapper=document.createElement('span');
      wrapper.className='note-image-item';
      img.parentNode.insertBefore(wrapper,img);
      wrapper.appendChild(img);
      const button=document.createElement('button');
      button.type='button';
      button.className='note-image-download';
      button.textContent='⬇';
      button.title='تحميل الصورة';
      button.setAttribute('aria-label','تحميل الصورة '+(index+1));
      button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();downloadImage(img,button,index+1)});
  ...
  function refresh(){
    queued=false;
    const notes=[...document.querySelectorAll('#list > .note')];
    const pages=Math.max(1,Math.ceil(notes.length/pageSize));
    page=Math.min(Math.max(1,page),pages);
    const start=(page-1)*pageSize,end=start+pageSize;
    notes.forEach((article,index)=>{
      article.hidden=index<start||index>=end;
      const pick=article.querySelector('.pick');
      if(pick){pick.title='تحديد الملاحظة #'+pick.value;pick.setAttribute('aria-label','تحديد الملاحظة #'+pick.value)}
      enhanceImages(article);
    });
    renderPager(notes.length);
    updateSelectionBar();
  }
  ...
  function preserveBulkSelectionAroundNoteTool(event){
    const tool=event.target.closest?.('.note-tool');
    if(!tool)return;
    const ids=new Set(selectedIds());
    setTimeout(()=>{
      document.querySelectorAll('.pick').forEach(x=>x.checked=ids.has(Number(x.value)));
      updateSelectionBar();
    },0);
  }
  ...
  function start(){
    ...
    ['search','statusFilter','typeFilter'].forEach(id=>{
      ...
      element.addEventListener(event,()=>{page=1;setTimeout(scheduleRefresh,0)});
    });
    document.addEventListener('change',event=>{if(event.target.matches?.('.pick'))updateSelectionBar()});
    document.addEventListener('click',preserveBulkSelectionAroundNoteTool,true);
    const list=document.getElementById('list');
    if(list)new MutationObserver(scheduleRefresh).observe(list,{childList:true});
    scheduleRefresh();
  }
